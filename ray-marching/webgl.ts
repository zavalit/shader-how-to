...
export default (
  ...
  const listenToMouseMove = (ev: MouseEvent)=> {
    ...
    MOUSE_COORDS.x = (ev.clientX - left) * devicePixelRatio / gl.drawingBufferWidth;
    MOUSE_COORDS.y = (height - fromTop) * devicePixelRatio / gl.drawingBufferHeight;
  ...
  const pLocs = Object.keys(PARAMS).reduce((acc, key) => {
    acc[key] = gl.getUniformLocation(prog, `u${key}`)
    return acc
  }, {}) 
  ...
  const step = (time: number) => {
    ...
    paramsKeys.forEach(key => {
      gl.uniform1f(pLocs[key], PARAMS[key]);
    })
